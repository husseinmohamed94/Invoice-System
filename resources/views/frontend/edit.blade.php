            ...
            <div class="card-body">
                ...
                <form  action="{{route('invoice.update',$invoice->id)}}" method="POST" class="form">
                            ...
                            <table class="table" id="invoice_details">
                                ...
                                @foreach($invoice->details as $item)
                                    <tr class="cloning_row" id="{{$loop->index}}">
                                        <td>@if($loop->index == 0 ){{'#'}}@else
                                        <button type="button" class="btn btn-danger btn-sm delgated-btn"><i class="fa fa-minus"></i></button>
                                         @endif</td>
                                        <td><input type="text" name="prouduct_name[{{$loop->index}}]" id="prouduct_name"  value = "{{old('prouduct_name',$item->prouduct_name)}}" class="prouduct_name form-control"></td>
                                        <td> 
                                        <select name="unit[{{$loop->index}}]" id="unit" class="unit form-control">
                                            <option value=""></option>
                                            <option value="price" {{$item->unit == 'price' ? 'selected' : ''}} >{{__('Frontend/frontend.price')}}</option>
                                        ...
                                        <td>
                                        <input type="number" step="0.01"  name="quantity[{{$loop->index}}]" id="quantity" value = "{{old('quantity',$item->quantity)}}" class="quantity form-control">
                                        @error('quantity')<span class="help-block text-danger">{{$message}}</span>@enderror
                                        </td>
                                        <td>
                                        <input type="number" step="0.01"  name="unit_price[{{$loop->index}}]" id="unit_price"  value = "{{old('unit_price',$item->unit_price)}}" class="unit_price form-control">
                                        @error('unit_price')<span class="help-block text-danger">{{$message}}</span>@enderror
                                        </td>
                                      
                                        <td>
                                        <input type="number" step="0.01"  name="row_sub_total[{{$loop->index}}]" value = "{{old('row_sub_total',$item->row_sub_total)}}" id="row_sub_total" class="row_sub_total form-control" readonly="readonly">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                           <div class="input-group mb-3">
                                            ...
                                            <option value="parcentage"  {{$invoice->discount_type == 'parcentage' ? 'selected' : ''}}>{{__('Frontend/frontend.parcentage')}}</option>
                                            </select>
                                                <div class="input-group-append">
                                                    <input type="number" step="0.01" name="discount_value"  value = "{{old('discount_value',$invoice->discount_value)}}" id="discount_value" class="discount_value form-control" value="0.00">
                                                </div>    
                                           </div>
                                        </td>
                                    </tr>
                    ...
                    <div class="text-right pt-3">
                        ...
                        <button class="btn btn-primary" name="save" type="submit">{{__('Frontend/frontend.update')}}</button>
